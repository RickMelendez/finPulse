        ...
        <CardContent>
          ...
          {insightData && (
            <div className="space-y-6">
              ...
              {/* Recommendations */}
              <div>
                <h3 className="font-heading text-sm font-semibold text-slate-800 mb-3">
                  Recommendations
                </h3>
                  ...
                  <div className="space-y-2">
                    {topCategories.slice(0, 6).map(([name, value]) => (
                      <div key={name} className="flex items-center gap-3">
                        <span className="font-body text-sm text-slate-600 w-28 truncate">
                          {name}
                        </span>
                        ...
                        <span className="font-heading text-sm font-semibold text-slate-800 w-20 text-right">
                          {fmt(value)}
                        </span>
                      </div>
                    ))}
                  ...
                  <div className="space-y-2">
                    {insightData.anomalies.map((a, i) => (
                      <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-amber-50 border border-amber-100">
                        ...
                        <div>
                          <p className="font-body text-sm text-slate-700">{a.description}</p>
          ...
          {budgetPlan && !generatePlan.isPending && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-income/5 rounded-xl p-4">
                  <p className="font-body text-xs text-slate-500 uppercase tracking-wider mb-1">Monthly Income</p>
                ...
                <div className="bg-expense/5 rounded-xl p-4">
                  <p className="font-body text-xs text-slate-500 uppercase tracking-wider mb-1">Current Expenses</p>
                ...
                <div className="bg-brand/5 rounded-xl p-4">
                  <p className="font-body text-xs text-slate-500 uppercase tracking-wider mb-1">Savings Target</p>
              ...
              <p className="font-body text-slate-700 leading-relaxed bg-brand/5 border-l-4 border-brand p-4 rounded-r-xl text-sm">
                {budgetPlan.summary}
              </p>
                ...
                <table className="w-full font-body text-sm">
                  <thead>
                    <tr className="border-b border-border">
                      {['Category', 'Current Spend', 'Recommended', '% of Income', 'Status', 'Reasoning'].map((h) => (
                        <th key={h} className="text-left py-2 px-3 text-xs font-semibold text-slate-400 uppercase tracking-wider whitespace-nowrap">
                          {h}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {budgetPlan.items.map((item) => (
                      <tr key={item.category} className="border-b border-border hover:bg-slate-50 transition-colors">
                        <td className="py-3 px-3 font-semibold text-slate-800 whitespace-nowrap">{item.category}</td>
                        <td className="py-3 px-3 text-slate-500">{fmt(item.currentMonthSpend)}</td>
                        <td className="py-3 px-3 font-semibold text-slate-800">{fmt(item.recommendedMonthly)}</td>
                        <td className="py-3 px-3 text-slate-500">{item.percentOfIncome.toFixed(0)}%</td>
                        ...
                        <td className="py-3 px-3 text-slate-500 text-xs max-w-[240px]">{item.reasoning}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
